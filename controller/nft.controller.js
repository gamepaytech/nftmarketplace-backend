...
const getNftById = async (req, res) => {
    try {
        ...
        const data = req.body.type === "inventory" ? await Nfts.nftDetails.findOne({ _id: nftId}): await Nfts.nftDetails.findOne({ $and: [{ _id: nftId},{  active:true } ] });
        res.send({
            data: data,
            msg: "Successfull",
        });
...
const buyNft = async (req,res) => {
    ...
    await models.users.findByIdAndUpdate({_id:ObjectId(req.user.userId)},      {
        $push: {
            activity: {
    ...
    await models.users.findByIdAndUpdate({_id:ObjectId(preSaleData.ownerId)},      {
        $push: {
            activity: {
    ...
    if(preSaleData.boughtId){
        await PresaleBoughtNft.findByIdAndUpdate({_id:ObjectId(preSaleData.boughtId)},{active:false})
    }
    res.status(201).json({
...
const sellNft = async(req,res) => {
    ...
    const {name,nftType,description,nftClass,gender,accessories,colour,others,breedCount,nftStatus,tierType,mintedBy,chikCount} = await Nfts.nftDetails.findById({_id:ObjectId(nftId)})
    nftData = {
        name,
        nftType,
        description,
...
const getNftByUserId = async (req, res) => {
    try {
        ...
        let total = 0;
        total =  await PresaleBoughtNft.find({$and:[{nftIdOwned:ObjectId(userId)},{active:true}]}).count()
        const findNfts = await PresaleBoughtNft.find({$and:[{nftIdOwned:ObjectId(userId)},{active:true}]}).sort({ createdAt:-1 }).populate({
            path: "nft"
